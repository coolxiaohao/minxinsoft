...
//照片上传
  ...
  Save(){
    ...
    _that = this;
    const url= urls + "/index.php/api/index/ruku";
    this.setState({
      loaded:true
    })
    ...
    return Promise.race([
      fetch(url, {
          ...
          body: 'shuzu='+JSON.stringify(goodsc)+'&imgs='+JSON.stringify(imgPickers)+'&saomiaoshijian='+timestamp+'&cangkudaihao='+this.props.navigation.state.params.cangkudaihao+"&user_token="+this.state.names.user_token+"&huoqukongbai="+this.state.ishuoqu,
        }),
        new Promise(function(resolve,reject){
            setTimeout(()=> reject(new Error('Network request timeout!')),10000)
        })])
