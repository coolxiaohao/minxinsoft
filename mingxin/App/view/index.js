...
export default class mingxin extends Component {
    ...
    return (
      <View style={styles.container}>
        <ScrollView showsVerticalScrollIndicator={false} style={{width:width}}>
          <View style={{alignItems: 'center',justifyContent: 'center',position:'relative',backgroundColor:'#ffffff'}}>
            <Image style={styles.titlebgStyles}
                   source={{uri:'indexbg'}}/>
           <TouchableOpacity activeOpacity = {0.5} style={{position:'absolute',top:10,right:20,width:40,height:40}} onPress={()=>{   
        ...
        {/* <Modal
          ...
          <Modals
            ...
            animationOut='fadeOut'
             >
               <TouchableOpacity activeOpacity = {1} onPress={()=>this.setState({isSetUp:false})} style={{flex:1,justifyContent:'flex-end',alignItems:'flex-end',backgroundColor:'rgba(0, 0, 0, 0.5)'}}>
                  {this.setUp()}
               </TouchableOpacity>
          </Modals>
          <Modals
            ...
            animationOut='fadeOut'
             >
             <TouchableOpacity activeOpacity = {1} onPress={()=>this.setState({isShare:false})} style={{flex:1,justifyContent:'flex-end',alignItems:'flex-end',backgroundColor:'rgba(0, 0, 0, 0.5)'}}>
                {this.sharePage()}
             </TouchableOpacity>
          </Modals>
  ...
  sharePage(){
    return(
      <View style={{width: width,height: 100,paddingBottom:20,backgroundColor: '#ffffff',borderRadius:10,flexDirection:'row',justifyContent:'space-around',alignItems:'center'}}>
  ...
  setUp(){
    ...
    return(
      <View style={{width: width,height: showDefault ? 160 : 120,backgroundColor: '#ffffff',borderRadius:10,justifyContent:'space-around',alignItems:'center',paddingBottom: 20}}>
        <Text onPress={()=>{
        ...
        {
          showDefault ? (<Text onPress={()=>{this.setState({isSetUp:false,}); this.props.navigation.navigate('Systemdefault')}}>{this.state.systemDefaultName}</Text>) : (null)
        }
        <Text onPress={()=>{
          zhijiemenus=[];
          gengdanmenus=[];
          ...
          AsyncStorage.removeItem('userInfo'); AsyncStorage.removeItem('systemDefault');
          if (this.props.navigation.state.params.call) {
            this.props.navigation.state.params.call(this.state.lan)
          }
          this.props.navigation.navigate('Login')}}>{this.state.logoutName}</Text>
      ...
      lists.push(
        <TouchableOpacity key={index} style={{flex:1}} activeOpacity = {0.5} onPress={()=>{   
          ...
          <View style={{alignItems: 'center',justifyContent: 'center',marginTop:10,marginBottom:30}}>
            <Image source={{uri:urls+val.head}} style={{width:30,height:30,resizeMode: 'contain'}}></Image>
  ...
  renderRow(rowData:string, sectionID:number,rowID: number){
    return (
      <TouchableOpacity onPress={()=>{   
        ...
        <View style={{alignItems: 'center',justifyContent: 'center',width: (width - 40)  / 3 - 2,marginTop:10,marginBottom:10}}>
          <Image source={{uri: urls+rowData.head}} style={{width:30,height:30,resizeMode: 'contain'}}></Image>
