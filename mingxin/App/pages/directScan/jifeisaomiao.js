    ...
    return (
      <View style={styles.container}>
        <ScrollView scrollEnabled={false} style={{paddingVertical: 0,}}>
          ...
          <Text style={{color: '#000000',fontSize: 14,flex: 0.6,textAlign:'center',}}>{this.state.saveYuan ? this.state.names.username : this.state.userName}</Text>
          <View style={styles.formStyles}>
          ...
          {/* <Modal
               ...
               onRequestClose={() => {}} >
                <TouchableOpacity onPress={()=>this.setState({showPic:false})}>
                  <Image source={{uri: imgPickers[picid].uri}} style={{width:width,height:height,alignSelf:'center',marginBottom: 10}}></Image>
                </TouchableOpacity>
            </Modal> */}
      </View>
    );
      ...
      .then((json) => {
        ...
        if (json.state === 'success') {
          ...
          if(json.msgcode == '004'){
            ...
            this.refs.toast.show(this.state.message296,3000);
            //alert('该数据为空')
          }else{
            this.refs.toast.show(json.message,3000);
            //alert(json.message)
  ...
  //条码信息渲染
   ...
   //照片上传
     ...
     Save(){
       ...
       _that = this;
       const url= urls + "/index.php/api/index/jifei";
       //console.error(this.state.names.user_token)
       this.setState({
         loaded:true
       })
       ...
       return Promise.race([
         fetch(url, {
             ...
             body: 'shuzu='+JSON.stringify(goodsc)+'&saomiaoshijian='+timestamp+'&imgs='+JSON.stringify(imgPickers)+"&yuangonghao="+this.state.ReceiveGongHao,
           }),
           new Promise(function(resolve,reject){
               setTimeout(()=> reject(new Error('Network request timeout!')),10000)
           })])
