...
export default class mingxin extends Component {
    ...
    return (
      <View style={styles.container}>
        <ScrollView scrollEnabled={false} style={{paddingVertical: 0,flex:1}}>
          ...
          <Text style={{color: '#000000',fontSize: 16,flex: 0.6,textAlign:'center',}}>{this.state.userName}</Text>
          <View style={styles.formStyles}>
  ...
  getTiaoMaData(e){
    ...
    const url= urls + "/index.php/api/index/jifeisaoma1?tiaoxingma="+e+"&yuangonghao="+this.state.ReceiveGongHao;
    return Promise.race([
        fetch(url),
        new Promise(function(resolve,reject){
            setTimeout(()=> reject(new Error('Network request timeout!')),10000)
      ...
      .then((json) => {
        ...
        if (json.state === 'success') {
           ...
           goodsc.push({"tiaoma":json.data.r49a007,"saomiaoshu":json.data.r50bm032,"kesaomiaoshu":json.data.r50bm032,"totalShuL":json.data.r49a006});
           _that.setState({
             dataSource: ds.cloneWithRows(goodsc),
             hangshu:goodsc.length,
             shuliang:'',
  ...
  //条码信息渲染
   ...
   //照片上传
     ...
     Save(){
       ...
       _that = this;
       const url= urls + "/index.php/api/index/buliangpin";
       //console.error(this.state.names.user_token)
       this.setState({
         loaded:true
       })
       ...
       return Promise.race([
             ...
             body: 'shuzu='+JSON.stringify(goodsc)+'&saomiaoshijian='+timestamp+'&gongxuhao='+this.props.navigation.state.params.dingdanhao+'&imgs='+JSON.stringify(imgPickers)+"&yuangonghao="+this.state.ReceiveGongHao,
           }),
           new Promise(function(resolve,reject){
               setTimeout(()=> reject(new Error('Network request timeout!')),10000)
           })])
