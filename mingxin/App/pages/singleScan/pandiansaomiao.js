...
export default class mingxin extends Component {
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <ScrollView scrollEnabled={true} style={{paddingVertical: 0,}}>
          ...
          <View style={styles.dataStyles}>
            <View style={styles.headerStyles}>
              ...
              <Text style={{flex:1,textAlign:'center',fontSize:12}}>{this.state.saomiaoName}</Text>
  ...
  getData(e){
    ...
    return Promise.race([
      ...
      .then((json) => {
        if (json.state === 'success') {
          //console.error(json.data.huopindaihao);
          goodsc.push({"tiaoma":e,"kucun":json.data.jiecunshuliang,"saomiaoshu":'1',"huopingdaihao":json.data.huopindaihao,"yanse":json.data.yanse,"huoqu":this.state.huoquValue});
...
//照片上传
  ...
  Save(){
    ...
    _that = this;
    const url= urls + "/index.php/api/index/pandian1";
    //console.error(this.state.names.user_token)
    this.setState({
      loaded:true
    })
    ...
    return Promise.race([
        fetch(url, {
          ...
          body: 'shuzu='+JSON.stringify(goodsc)+'&saomiaoshijian='+timestamp+'&imgs='+JSON.stringify(imgPickers)+'&cangkudaihao='+this.props.navigation.state.params.cangkudaihao+"&user_token="+this.state.names.user_token+"&huoqukongbai="+this.state.ishuoqu,
        }),
        new Promise(function(resolve,reject){
            setTimeout(()=> reject(new Error('Network request timeout!')),10000)
        })])
